...
public class Instructions
{
	...
	private static class DisasmEntry
	{
		...
		public DisasmEntry(InstructionFmt format, BiConsumer<Simulator, int[]> executor)
		{
			this.format = format;
	...
	static
	{
		instructionMap[0] = new DisasmEntry(InstructionFmt.R, OpSpecial.executor);//special
		instructionMap[1] = new DisasmEntry(InstructionFmt.R, OpRegimm.executor);//regimm

		instructionMap[2] = new DisasmEntry(InstructionFmt.J, (sim, p) -> //j
			sim.scheduleAbsoluteJump(p[1] << 2, 0x0fffffff));

		instructionMap[3] = new DisasmEntry(InstructionFmt.J, (sim, p) -> {//jal
			sim.scheduleAbsoluteJump(p[1] << 2, 0x0fffffff);
			sim.reg.set(31, sim.getPC() + 8);
		});

		instructionMap[4] = new DisasmEntry(InstructionFmt.I, (sim, p) -> {//beq
			if(sim.reg.get(p[0]) == sim.reg.get(p[1])) sim.scheduleRelativeJump((p[2] + 1) << 2);
		});

		instructionMap[5] = new DisasmEntry(InstructionFmt.I, (sim, p) -> {//bne
			if(sim.reg.get(p[0]) != sim.reg.get(p[1])) sim.scheduleRelativeJump((p[2] + 1) << 2);
		});

		instructionMap[6] = new DisasmEntry(InstructionFmt.I, (sim, p) -> {//blez
			if(sim.reg.get(p[0]) <= 0) sim.scheduleRelativeJump((p[2] + 1) << 2);
		});

		instructionMap[7] = new DisasmEntry(InstructionFmt.I, (sim, p) -> {//bgtz
			if(sim.reg.get(p[0]) > 0) sim.scheduleRelativeJump((p[2] + 1) << 2);
		});

		instructionMap[8] = new DisasmEntry(InstructionFmt.I, (sim, p) -> {//addi
		...
		instructionMap[9] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //addiu
			sim.reg.set(p[1], sim.reg.get(p[0]) + p[2]));

		instructionMap[10] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //slti
			sim.reg.set(p[1], sim.reg.get(p[0]) < p[2]? 1: 0));

		instructionMap[11] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //sltiu
			sim.reg.set(p[1], ((long)sim.reg.get(p[0]) & 0xffffffffL) < ((long)p[2] & 0xffffffffL)? 1: 0));

		instructionMap[12] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //andi
			sim.reg.set(p[1], sim.reg.get(p[0]) & (p[2] & 0x0000ffff)));
		
		instructionMap[13] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //ori
			sim.reg.set(p[1], sim.reg.get(p[0]) | (p[2] & 0x0000ffff)));
		
		instructionMap[14] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //xori
			sim.reg.set(p[1], sim.reg.get(p[0]) ^ (p[2] & 0x0000ffff)));
		
		instructionMap[15] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //lui
		...
		instructionMap[20] = new DisasmEntry(InstructionFmt.I, (sim, p) -> { //beql
		...
		instructionMap[21] = new DisasmEntry(InstructionFmt.I, (sim, p) -> { //bnel
			if(sim.reg.get(p[0]) != sim.reg.get(p[1]))
				sim.scheduleRelativeJump((p[2] + 1) << 2);
			else
				sim.scheduleRelativeJump(8, 12);
		...
		instructionMap[22] = new DisasmEntry(InstructionFmt.I, (sim, p) -> { //blezl
		...
		instructionMap[23] = new DisasmEntry(InstructionFmt.I, (sim, p) -> { //bgtzl
			if(sim.reg.get(p[0]) > 0)
				sim.scheduleRelativeJump((p[2] + 1) << 2);
			else
				sim.scheduleRelativeJump(8, 12);
		...
		instructionMap[28] = new DisasmEntry(InstructionFmt.R, OpSpecial2.executor);
		instructionMap[29] = null;
		instructionMap[30] = null;
		instructionMap[31] = null;
		
		instructionMap[32] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //lb
			sim.reg.set(p[1], sim.mem.readByte(sim.reg.get(p[0]) + p[2])));
		instructionMap[33] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //lh
			sim.reg.set(p[1], sim.mem.readHalfWord(sim.reg.get(p[0]) + p[2])));
		instructionMap[34] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //lwl
			sim.reg.set(p[1], sim.mem.readLeft(sim.reg.get(p[0]) + p[2], sim.reg.get(p[1]))));
		instructionMap[35] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //lw
			sim.reg.set(p[1], sim.mem.readWord(sim.reg.get(p[0]) + p[2])));
		instructionMap[36] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //lbu
			sim.reg.set(p[1], sim.mem.readByteUnsigned(sim.reg.get(p[0]) + p[2])));
		instructionMap[33] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //lhu
			sim.reg.set(p[1], sim.mem.readHalfWordUnsigned(sim.reg.get(p[0]) + p[2])));
		instructionMap[38] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //lwr
			sim.reg.set(p[1], sim.mem.readRight(sim.reg.get(p[0]) + p[2], sim.reg.get(p[1]))));
		instructionMap[39] = null;
		
		instructionMap[40] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //sb
			sim.mem.writeByte(sim.reg.get(p[0]) + p[2], sim.reg.get(p[1])));
		instructionMap[41] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //sh
			sim.mem.writeHalfWord(sim.reg.get(p[0]) + p[2], sim.reg.get(p[1])));
		instructionMap[42] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //swl
			sim.mem.writeRight(sim.reg.get(p[0]) + p[2], sim.reg.get(p[1])));
		instructionMap[43] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //sw
			sim.mem.writeWord(sim.reg.get(p[0]) + p[2], sim.reg.get(p[1])));
		instructionMap[44] = null;
		instructionMap[45] = null;
		instructionMap[46] = new DisasmEntry(InstructionFmt.I, (sim, p) -> //swr
			sim.mem.writeRight(sim.reg.get(p[0]) + p[2], sim.reg.get(p[1])));
